export function init() {
    bindAgendaSettingsIndexRowClick();
}

function bindAgendaSettingsIndexRowClick() {
    const grid = document.getElementById('agenda-settings-grid');
    if (!grid) {
        return;
    }

    const wrapper = document.getElementById('agenda-settings-grid-wrapper');
    const linkSelector = wrapper?.dataset?.rowClickLinkSelector || 'a[title="Visualizar"]';
