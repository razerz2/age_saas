                        ...
                        <div class="row g-3">
                                ...
                                <select id="pais" name="pais_id" class="form-select">
                                    ...
                                    @foreach($paises as $pais)
                                        <option value="{{ $pais->id_pais }}"
                                            @selected($loc && $loc->pais_id == $pais->id_pais)>
                                            {{ $pais->nome }}
                                        </option>
                                    @endforeach
                                </select>
                                    ...
                                    @if($estados->isNotEmpty())
                                        @foreach($estados as $estado)
                                            <option value="{{ $estado->id_estado }}"
                                                @selected($loc && $loc->estado_id == $estado->id_estado)>
                                                {{ $estado->nome_estado }}
                                            </option>
                                        @endforeach
                                    @else
                                    ...
                                    @if($cidades->isNotEmpty())
                                        @foreach($cidades as $cidade)
                                            <option value="{{ $cidade->id_cidade }}"
                                                @selected($loc && $loc->cidade_id == $cidade->id_cidade)>
                                                {{ $cidade->nome_cidade }}
                                            </option>
                                        @endforeach
                                    @else
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="db_host" class="form-control"
                                    value="{{ old('db_host', $tenant->db_host) }}" required>
                            </div>

                            <div class="col-md-6">
                                <label class="form-label">DB Name *</label>
                                <input type="text" name="db_name" class="form-control"
                                    value="{{ old('db_name', $tenant->db_name) }}" required>
                            </div>

                            <div class="col-md-6">
                                <label class="form-label">DB User *</label>
                                <input type="text" name="db_username" class="form-control"
                                    value="{{ old('db_username', $tenant->db_username) }}" required>
                            </div>

                            <div class="col-md-6">
                                <label class="form-label">DB Password *</label>
                                <input type="password" name="db_password" class="form-control"
                                    value="{{ old('db_password', $tenant->db_password) }}" required>
                            </div>
                        </div>
...
<script>
    ...
    if (pais && estado && cidade) {
        pais.addEventListener('change', function () {
            const urlEstados = "{{ route('Platform.api.estados', ['pais' => '__ID__']) }}".replace('__ID__', this.value);
            fetch(urlEstados)
                .then(r => r.json())
                .then(data => {
                    estado.innerHTML = '<option value="">Selecione...</option>';
                    data.forEach(e => estado.innerHTML += `<option value="${e.id_estado}">${e.nome_estado}</option>`);
                    cidade.innerHTML = '<option value="">Selecione o estado primeiro</option>';
                });
        });

        estado.addEventListener('change', function () {
            const urlCidades = "{{ route('Platform.api.cidades', ['estado' => '__ID__']) }}".replace('__ID__', this.value);
            fetch(urlCidades)
                .then(r => r.json())
                .then(data => {
                    cidade.innerHTML = '<option value="">Selecione...</option>';
                    data.forEach(c => cidade.innerHTML += `<option value="${c.id_cidade}">${c.nome_cidade}</option>`);
                });
        });
    }
</script>
