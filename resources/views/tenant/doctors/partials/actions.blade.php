<div class="actions-wrap">
    ...
    <form
        id="doctors-delete-form-{{ $doctor->id }}"
        action="{{ workspace_route('tenant.doctors.destroy', $doctor->id) }}"
        method="POST"
        class="inline-flex delete-doctor-form"
        data-doctor-name="{{ optional($doctor->user)->name ?? '' }}"
    ...
    >
        ...
        <button
            ...
            data-delete-message="Tem certeza que deseja excluir {{ optional($doctor->user)->name ?? 'este médico' }}?"
            onclick="event.stopPropagation()"
