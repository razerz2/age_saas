    ...
    <div class="space-y-6">
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="flex flex-wrap items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <li>/</li>
                        <li><a href="{{ workspace_route('tenant.integrations.index') }}" class="hover:text-gray-800 dark:hover:text-white">Integrações</a></li>
                        <li>/</li>
                        <li class="font-medium text-gray-900 dark:text-white">Criar</li>
                    </ol>
                </nav>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Nova Integração Genérica</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Use este cadastro para feature flags/configurações complementares por tenant.</p>
            </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Dados da Integração</h2>
            </div>
            ...
            <form action="{{ workspace_route('tenant.integrations.store') }}" method="POST" class="p-6 space-y-6">
                ...
                <div>
                    <label for="config" class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Configuração (JSON)</label>
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Informe JSON válido (objeto ou lista). Deixe vazio para sem configuração.</p>
                    @error('config')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-300">
                    Google Calendar e Apple Calendar oficiais são gerenciados pelas telas dedicadas por médico; este formulário não substitui o fluxo de conexão.
                </div>
                ...
                <div class="flex flex-wrap items-center justify-end gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                    ...
                    <button type="submit" class="rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700">Salvar integração</button>
                </div>
            </form>
        </div>
    </div>
