...
@section('title', 'Editar Integracao')
...
@section('content')
    <div class="space-y-6">
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="flex flex-wrap items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <li>/</li>
                        <li class="font-medium text-gray-900 dark:text-white">Editar</li>
                    </ol>
                </nav>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Editar Integracao Generica</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Atualize os dados complementares da integracao.</p>
            </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <form action="{{ workspace_route('tenant.integrations.update', $integration->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <input id="key" name="key" type="text" value="{{ old('key', $integration->key) }}" required
                    ...
                    <div>
                        ...
                        <select id="is_enabled" name="is_enabled"
                                class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                            <option value="1" {{ old('is_enabled', (string) (int) $integration->is_enabled) === '1' ? 'selected' : '' }}>Habilitado</option>
                            <option value="0" {{ old('is_enabled', (string) (int) $integration->is_enabled) === '0' ? 'selected' : '' }}>Desabilitado</option>
                        </select>
                        @error('is_enabled')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea id="config" name="config" rows="10"
                              class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 font-mono text-sm text-gray-900 shadow-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-600 dark:bg-gray-700 dark:text-white">{{ old('config', is_array($integration->config) ? json_encode($integration->config, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : $integration->config) }}</textarea>
                ...
                <div class="flex flex-wrap items-center justify-end gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                    ...
                    <button type="submit" class="rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700">Atualizar integracao</button>
                </div>
            </form>
        </div>
    </div>
