...
<head>
    <meta charset="utf-8">
    <title>Relatorio de Recorrencias</title>
    <style>
...
<body>
    <h1>Relatorio de Recorrencias</h1>
    ...
    <table>
        ...
        <tbody>
            @forelse($rows as $row)
                <tr>
                    <td>{{ $row->doctor_name ?? ($row->doctor->user->name ?? 'N/A') }}</td>
                    <td>{{ $row->patient_name ?? ($row->patient->full_name ?? 'N/A') }}</td>
                    <td>{{ $row->appointment_type_name ?? ($row->appointmentType->name ?? 'N/A') }}</td>
                    <td>{{ $row->start_date ? $row->start_date->format('d/m/Y') : '-' }}</td>
                    <td>{{ ($row->active ?? false) ? 'Ativo' : 'Inativo' }}</td>
