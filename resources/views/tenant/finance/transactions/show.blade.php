...
@section('content')
    ...
    <div class="page-header mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white mb-2">Detalhes da Transação</h1>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 max-w-3xl">
        <div class="p-6">
            <div class="flex items-start justify-between mb-6">
                <div>
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Transação #{{ substr($transaction->id, 0, 8) }}</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Detalhes completos da transação</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-900 dark:text-gray-100">{{ $transaction->account->name ?? 'N/A' }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Categoria</p>
                    <p class="text-sm text-gray-900 dark:text-gray-100">{{ $transaction->category->name ?? 'N/A' }}</p>
                </div>
            </div>
            ...
            @if($transaction->patient)
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-900 dark:text-gray-100">{{ $transaction->patient->full_name }}</p>
                </div>
            @endif
            ...
            @if($transaction->doctor)
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-900 dark:text-gray-100">{{ $transaction->doctor->user->name ?? 'N/A' }}</p>
                </div>
            @endif
            ...
            @if($transaction->appointment)
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-900 dark:text-gray-100">{{ $transaction->appointment->starts_at->format('d/m/Y H:i') }}</p>
                </div>
            @endif
            ...
            @if($transaction->creator)
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-900 dark:text-gray-100">{{ $transaction->creator->name }}</p>
                </div>
            @endif
