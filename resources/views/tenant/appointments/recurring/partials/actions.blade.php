...
<div class="actions-wrap">
    <a
        href="{{ workspace_route('tenant.recurring-appointments.show', ['id' => $recurring->id]) }}"
        title="Ver"
    ...
    <a
        href="{{ workspace_route('tenant.recurring-appointments.edit', ['id' => $recurring->id]) }}"
        title="Editar"
        onclick="event.stopPropagation()"
    ...
    @php($tenantUser = auth('tenant')->user())
    @if ($tenantUser && $tenantUser->can('delete', $recurring))
        <form
            id="recurring-appointments-delete-form-{{ $recurring->id }}"
            action="{{ workspace_route('tenant.recurring-appointments.destroy', ['id' => $recurring->id]) }}"
            method="POST"
        ...
        >
            ...
            <button
                ...
                data-delete-form="#recurring-appointments-delete-form-{{ $recurring->id }}"
                data-delete-title="Excluir agendamento recorrente"
                data-delete-message="Tem certeza que deseja excluir este agendamento recorrente? Esta ação não pode ser desfeita."
                onclick="event.stopPropagation()"
