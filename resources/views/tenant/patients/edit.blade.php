    ...
    <div class="max-w-6xl mx-auto">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                <h4 class="text-xl font-semibold text-gray-900 dark:text-white">Editar Paciente</h4>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Atualize os dados abaixo para editar o paciente</p>
            </div>

            <form action="{{ workspace_route('tenant.patients.update', $patient->id) }}" method="POST" dusk="patient-form" class="p-6 space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="is_active" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white @error('is_active') border-red-500 @enderror">
                                <option value="1" {{ (string) old('is_active', $patient->is_active ? '1' : '0') === '1' ? 'selected' : '' }}>Ativo</option>
                                <option value="0" {{ (string) old('is_active', $patient->is_active ? '1' : '0') === '0' ? 'selected' : '' }}>Inativo</option>
                            </select>
                            @error('is_active')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
