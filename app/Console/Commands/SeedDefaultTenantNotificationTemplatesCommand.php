...
class SeedDefaultTenantNotificationTemplatesCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tenants:seed-default-notification-templates
     ...
     * @var string
     */
    protected $description = 'Copia o baseline Platform de tenant_default_notification_templates para notification_templates dos tenants.';
    ...
    public function handle(): int
    {
        ...
        $tenants = $this->resolveTenants($tenantOption, $allTenants);
        if ($tenants->isEmpty()) {
            return self::FAILURE;
        }
        ...
        if ($dryRun) {
            ...
            $command = 'php artisan tenants:seed-default-notification-templates ';
            $command .= $allTenants ? '--all-tenants' : '--tenant=' . $tenantOption;
            $command .= ' --apply';
            if ($overwrite) {
                $command .= ' --overwrite';
            }
