...
class SeedAppointmentsForDate extends Command
{
    protected $signature = 'tenant:appointments:seed-date
                            {--tenant= : Slug (subdomain) de um tenant especifico}
                            {--all-tenants : Executa em todos os tenants}
                            {--date= : Data fixa para geracao (YYYY-MM-DD)}
    ...
    protected $description = 'Gera agendamentos ficticios para uma data especifica.';
    ...
    public function handle(): int
    {
        ...
        if (trim($dateOption) === '') {
            $this->error('A opcao --date=YYYY-MM-DD e obrigatoria.');
            return self::FAILURE;
        }

        try {
            $targetDate = CarbonImmutable::parse($dateOption)->startOfDay();
        } catch (Throwable) {
            $this->error('Data invalida em --date. Use formato YYYY-MM-DD.');
            return self::FAILURE;
        }
        ...
        foreach ($tenants as $tenant) {
            ...
            try {
                $tenant->makeCurrent();
                $stats = $this->seedForTenant($count, $durationFallback, $tag, $targetDate, $dryRun);
