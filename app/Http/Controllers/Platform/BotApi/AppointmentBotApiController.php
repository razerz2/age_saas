...
class AppointmentBotApiController extends Controller
{
        ...
        try {
            ...
            return (function () use ($validated, $tenant) {
                ...
                if (!$doctor || !$doctor->user || $doctor->user->status !== 'active') {
                    return response()->json([
                        'success' => false,
                        'error' => $this->professionalSingular() . ' não encontrado ou inativo'
                    ], 404);
                }
                ...
                if (!$calendar) {
                    return response()->json([
                        'success' => false,
                        'error' => 'Calendário não encontrado para este ' . $this->professionalSingularLower($doctor)
                    ], 404);
                }
    ...
    private function validateAvailableSlot($doctorId, Carbon $startsAt, Carbon $endsAt)
    {
        ...
        if ($businessHours->isEmpty()) {
            ...
            return 'O ' . $this->professionalSingularLower($doctor) . ' não realiza atendimento em ' . $weekdayNames[$weekday];
        }
        ...
        if (!$isWithinBusinessHours) {
            return 'O horário selecionado está fora do horário de atendimento do ' . $this->professionalSingularLower($doctor);
        }
