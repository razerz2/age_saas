...
class StoreDoctorRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            // Verificar se já existe um médico com o mesmo user_id
            $existingDoctorByUser = Doctor::where('user_id', $this->user_id)->first();
            if ($existingDoctorByUser) {
                $validator->errors()->add('user_id', 'Este usuário já está cadastrado como médico.');
            }

            // Verificar se já existe um médico com o mesmo CRM (número + estado)
